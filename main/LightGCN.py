...
class LightGCN(object):
    ...
    def _init_weights(self):
        ...
        Xf_path = "citeseer.128.train.bin.f"
        Xb_path = "citeseer.128.train.bin.b"

        n, d = 3312, 64
        Xf = np.fromfile(Xf_path, dtype=np.float32).reshape((n, d))
        Xb = np.fromfile(Xb_path, dtype=np.float32).reshape((n, d))

        print(f"Xf size: {Xf.shape}")
        print(f"Xb size: {Xb.shape}")

        combined_embeddings = np.concatenate((Xf, Xb), axis=1) 
        print(f"Combined embeddings size: {combined_embeddings.shape}")

        combined_embeddings -= combined_embeddings.mean(axis=0)
        combined_embeddings_std = combined_embeddings.std(axis=0) + 1e-6 
        normalized_embeddings = combined_embeddings / combined_embeddings_std 

        print(f"Normalized embeddings size: {normalized_embeddings.shape}")

        fan_in = d * 2 
        fan_out = d * 2 
        std = np.sqrt(2 / (fan_in + fan_out))

        scaled_embeddings = normalized_embeddings * std
        ...
        if self.pretrain_data is None:
            ...
            all_weights['item_embedding'] = tf.Variable(scaled_embeddings, name='item_embedding')
    ...
    def _create_ngcf_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            # Gaussian noise
            # noise = tf.random.normal(shape=tf.shape(ego_embeddings), mean=0.0, stddev=1e-5)
