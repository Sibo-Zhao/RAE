...
def gen_emb(adj, F, k, t, kappa, rules):
    if kappa > 0 and F.shape[1] > 10000:
        ...
        Xf, Y, Xb, Y = gen_emb_pane(adj, F, k, t, rules) 
...
def gen_emb_pane(adj, features, k, t, rules):
    print("running PANE...")
    n = adj.shape[0]
    d = features.shape[1]
    t1 = time.time()
        ...
        with open(args.rules, 'r') as f:
            for line in f:
                ...
                if len(nums) == 2 or len(nums) == 3:
                    rules.append(tuple(nums))
    
    return adj, features, rules
...
def save_emb(Xf, Xb, Yf, Yb, args):
    ...
    with open(emb_file + ".f", "wb") as fout:
        np.asarray(Xf, dtype=np.float32).tofile(fout)

    with open(emb_file + ".b", "wb") as fout:
        np.asarray(Xb, dtype=np.float32).tofile(fout) 

    with open(attr_emb_file + ".f", "wb") as fout:
        np.asarray(Yf, dtype=np.float32).tofile(fout)  

    with open(attr_emb_file + ".b", "wb") as fout:
        np.asarray(Yb, dtype=np.float32).tofile(fout) 


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process...')
    parser.add_argument('--data', type=str, help='graph dataset name')
    parser.add_argument('--rules', type=str, default='rules', help='path to GAR rules file')
    ...
    print("loading data...")
    adj, features, rules = load_data(args)

    print("processing...")
    start_time = time.time()
    Xf, Yf, Xb, Yb = gen_emb(adj, features, args.d, args.t, args.kappa, rules)
